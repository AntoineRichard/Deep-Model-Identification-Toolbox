...
def MSE(y, y_, axis=0):
    mse = tf.reduce_mean((y-y_)**2, axis=axis)
    return mse

def RMSE(y, y_, axis=0):
    rmse = tf.sqrt(tf.reduce_mean((y-y_)**2,axis=axis))
    return rmse

def Gaussian_NLL(y, mu, sigma, axis=0):
    logprob = -tf.math.log(sigma) - 0.5*tf.math.log(2*np.pi) - 0.5*((y-mu)/sigma)**2
    loss = tf.reduce_mean(-logprob, axis=axis)
    return loss

def Gaussian_NLL_logvar(y, mu, logvar, axis=0):
    log_liklihood = 0.5 * (
        -tf.exp(-logvar)*(mu-y)**2 - tf.math.log(2*tf.constant(np.pi, dtype=logvar.dtype)) - logvar
    )
    loss = tf.reduce_mean(-log_liklihood, axis=axis)
    return loss
