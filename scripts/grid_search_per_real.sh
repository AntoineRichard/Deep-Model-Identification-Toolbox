...
to_file="$output_root/gridsearch_per_real.txt"
save_suffix="$output_root/results/GS/PER_real"
tb_suffix="$output_root/tensorboard/GS/PER_real"
...
do
	...
	do
		...
		do
			...
			echo python3 ${python_script} --train_data ${training_set} --val_data ${validation_set} --test_data ${test_set} --batch_size ${batch_size} --val_batch_size ${batch_val_size} --test_batch_size ${batch_test_size} --input_dim 5 --output_dim 3 --dropout ${drop_rate} --model ${model} --learning_rate ${learning_rate} --timestamp_idx 0 --output ${save_suffix}/0${alpha}_0${beta}/r${run} --tb_dir ${tb_suffix} --tb_log_name 0${alpha}_0${beta}-r${run} --reader_mode classic --priorization PER --max_sequence_size ${used_points} --trajectory_length ${traj_length} --update_batchsize 50000 --per_refresh_rate ${per_refresh} --alpha 0.${alpha} --beta 0.${beta} --max_iterations ${iterations} >> ${to_file}
		done
	done
done
