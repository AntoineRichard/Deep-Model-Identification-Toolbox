...
def get_statistics(args):
    ...
    f1.tight_layout()
    f2.tight_layout()
    f1.subplots_adjust(left=0.1, top=0.95)
    f2.subplots_adjust(left=0.1, top=0.95)
    ...
    for row_pos, ds in enumerate(zip(datasets,datasets_path)):
        ...
        for col_pos, md in enumerate(zip(models,models_path)):
            ...
            ax1[row_pos][col_pos].imshow(ss_mat,cmap='jet',origin='lower')
            ax1[row_pos][col_pos].colorbar()
            ax1[row_pos][col_pos].ylabel('Learning Rates')
            ax1[row_pos][col_pos].xlabel('Activations')
            ax1[row_pos][col_pos].yticks(list(range(bound_1)),param_1_list)
            ax1[row_pos][col_pos].xticks(list(range(bound_2)),param_2_list)
            ax1[row_pos][col_pos].xticks(rotation=45)
            # MultiStep Accuracy Map
            ax2[row_pos][col_pos].imshow(ms_mat,cmap='jet',origin='lower')
            ax2[row_pos][col_pos].colorbar()
            ax2[row_pos][col_pos].ylabel('Learning Rates')
            ax2[row_pos][col_pos].xlabel('Activations')
            ax2[row_pos][col_pos].yticks(list(range(bound_1)),param_1_list)
            ax2[row_pos][col_pos].xticks(list(range(bound_2)),param_2_list)
            ax2[row_pos][col_pos].xticks(rotation=45)
