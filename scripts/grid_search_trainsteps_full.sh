...
trainsteps="5000 10000 20000 30000 40000 60000 80000 100000 150000 200000 300000 400000"
...
to_file="$output_root/gridsearch_trainsteps.txt"
save_suffix="$output_root/results/GS/TrainStepsSearch"
tb_suffix="$output_root/tensorboard/GS/TrainStepsSearch"
...
do
	...
	do
		...
		for steps in ${trainsteps}
		do
			for run in $(seq 0 $run_max)
			do
				mkdir -p ${save_suffix}/${model_suffix}/${steps}/r${run}
				echo python3 ${python_script} --train_data ${training_set} --val_data ${validation_set} --test_data ${test_set} --batch_size ${batch_size} --val_batch_size ${batch_val_size} --input_dim 5 --output_dim 3 --dropout ${drop_rate} --model ${model} --learning_rate ${lr} --timestamp_idx 0 --output ${save_suffix}/${model_suffix}/${steps}/r${run} --tb_dir ${tb_suffix}/${model_suffix} --tb_log_name ${steps}-r${run} --reader_mode ${mode} --trajectory_length=${traj_length} --max_iterations ${steps} --max_sequence_size ${used_points}>> ${to_file}
			done
		done
	done
done
...
do
	...
	do
		...
		do
			...
			do
				mkdir -p ${save_suffix}/${model_suffix}/${steps}/r${run}
				echo python3 ${python_script} --train_data ${training_set} --val_ratio 0.1 --test_ratio 0.1 --batch_size ${batch_size} --val_batch_size 1000 --input_dim 8 --output_dim 4 --dropout ${drop_rate} --model ${model} --learning_rate ${lr} --output ${save_suffix}/${model_suffix}/${steps}/r${run} --tb_dir ${tb_suffix}/${model_suffix} --tb_log_name ${steps}-r${run} --reader_mode ${mode} --trajectory_length=${traj_length} --max_iterations ${steps} --max_sequence_size ${used_points}>> ${to_file}
			done
		done
	done
done
